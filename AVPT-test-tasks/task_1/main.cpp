...
constexpr short high_bit_chmp [] = {0xFF,   0x7F,   0x3F,   0x1F,
                                    0xF,    0x7,    0x3,    0x1};

short getdata(char *buf, int start, int len) {
    constexpr size_t to_bit {8};
    short data {0};
    //Определяем индекс символа буффера в котором находится
    size_t start_index {std::strlen(buf) - start/to_bit - 1},       //младший бит
           end_index   {std::strlen(buf) - (start+len)/to_bit - 1}; //старший бит
    if (!((start+len)%to_bit)) end_index++;
    ...
    for (int i = start_index; i >= end_index; i--)
        ...
        data |= buf[i] << (start_index-i)*to_bit >> start%to_bit;

    //накладываем маску, чтобы обрезать лишние старшие биты
    data &= high_bit_mp[16-len];
...
void putdata(char *buf, int start, int len, short value){
    ...
    //Собираем 16-ти битное число из 1-3-х 8-ми битных
    for (int i = start_index; i >= end_index; i--) {
        buf[i] ^= high_bit_chmp[to_bit-start%to_bit-1];
//        buf[i] = static_cast<char>((value >> (start_index-i)*to_bit )<< start%to_bit);// >> (start_index-i)*to_bit);
        buf[i] ^= value << start%to_bit >> (start_index-i)*to_bit;
    }//
...
int main()
{
    ...
    //01100001 01000001 01|100010 010000|10 01100011 01000011 01100100 01000100 01100101 01000101
    //
    //                   01011010 01011010 до
    //                   01100010 01000010 после
    //                   00100010 01000010
    //                   01111000 00011010

    char *buf {"aAbBcCdDeE\0"};
    short r = getdata(buf, 50, 12);
    bit = r;
    std::cout << r << std::endl;

    char *new_buf = new char[] {"aAZZcCdDeE\0"};
